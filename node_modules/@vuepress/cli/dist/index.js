...
var resolveAppConfig = ({
  ...
  if (appConfig.bundler === void 0 || appConfig.theme === void 0) {
    if (appConfig.bundler === void 0 ){
      logger.error('bundler is null')
    }
    if (appConfig.theme === void 0 ){
...
import {
  ...
  preparePageChunk as preparePageChunk2,
  preparePageComponent as preparePageComponent2,
  prepareRoutes as prepareRoutes2
} from "@vuepress/core";
var handlePageChange = async (app, filePath) => {
  ...
  await preparePageChunk2(app, pageNew);
  ...
  if (isPathChanged || isRouteMetaChanged) {
    await prepareRoutes2(app);
  }
  return [pageOld, pageNew];
};

// src/commands/dev/handlePageUnlink.ts
import { prepareRoutes as prepareRoutes3 } from "@vuepress/core";
...
var createDev = (defaultAppConfig) => {
  const dev = async (sourceDir = ".", commandOptions = {}) => {
    ...
    app.use(transformUserConfigToPlugin(userConfig, cliAppConfig.source));
    if (commandOptions.cleanTemp === true) {
      await withSpinner2("Cleaning temp")(async () => fs6.remove(app.dir.temp()));
    }
    if (commandOptions.cleanCache === true) {
      await withSpinner2("Cleaning cache")(
        async () => fs6.remove(app.dir.cache())
      );
    }
...
import { logger as logger7, ora } from "@vuepress/utils";
...
import { colors as colors5, logger as logger8 } from "@vuepress/utils";
import { cac } from "cac";
...
var cli = (defaultAppConfig = {}) => {
  ...
  if (program.matchedCommand) {
    ...
    program.runMatchedCommand().catch((err) => {
      logger8.error(err instanceof Error ? colors5.red(err.stack) : err);
      process9.exit(1);
    });
  } else {
    program.outputHelp();
